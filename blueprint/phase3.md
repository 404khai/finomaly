## Phase 3: Autoencoder Training & Redis Caching (Offline) **Objective:** Train the Autoencoder for point-anomalies and push GNN embeddings to Redis for real-time lookup. 

### Action Items: 
1. Write src/models/autoencoder.py: 
- Create a standard asymmetric PyTorch Autoencoder. 
- Input: Transaction tabular features. 
- Output: Reconstructed features. Loss function: Mean Squared Error (MSE). 

2. Write src/models/train_autoencoder.py: 
- Train only on normal transactions. 
- Calculate the 95th percentile of reconstruction error on the validation set. This becomes our anomaly_threshold. 

3. Write scripts/cache_embeddings.py: 
